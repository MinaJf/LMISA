    ...
    def __init__(self, n_class, n_layer, root_filters, kernal_size=3, pool_size=2, use_bn=True, use_res=True,
        ...
        for layer in range(n_layer):
            ...
            if layer < n_layer - 1:
                ...
                pool = layers.MaxPool3D([2,2,2], padding=padding)
        ...
        for layer in range(n_layer - 2, -1, -1):
            ...
            up = _UpSampling(filters, kernal_size, [2, 2, 2], 'up_%d' % layer, concat_or_add)
            self.up_layers[dict_key] = up

        stddev = np.sqrt(2 / (kernal_size ** 2 * root_filters))
        # stddev = 0.02
        self.conv_out1 = layers.Conv3D(1, 1, padding=padding, use_bias=False,
                                      kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                      name='conv_out1')
        self.conv_out2 = layers.Conv3D(5, 1, padding=padding, use_bias=False,
                                      kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                      name='conv_out2')
...
class _DownSev(Model):
    def __init__(self, filters, kernel_size, name, use_bn=True, use_res=True, padding='SAME', use_bias=True):
        ...
        self.conv1 = layers.Conv3D(filters, 7, padding=padding, use_bias=use_bias,
                                   kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                   name='conv2')
...
class Gen_encoder(Model):
    def __init__(self, n_class, n_layer, root_filters, kernal_size=3, pool_size=2, use_bn=True, use_res=True,
        ...
        for layer in range(n_layer):
            ...
            if layer < n_layer - 1:
                # if layer == 1:
                #     pool = layers.MaxPool3D([2,1,1], padding=padding)
                #     self.max_pools[dict_key] = pool
                # else:
                pool = layers.MaxPool3D([2,2,2], padding=padding)
                self.max_pools[dict_key] = pool
...
class Gen_decoder(Model):
    def __init__(self, n_class, n_layer, root_filters, kernal_size=3, pool_size=2, use_bn=True, use_res=True,
        ...
        for layer in range(n_layer - 2, -1, -1):
            ...
            dict_key = str(n_layer - layer - 1)
            # if layer == (n_layer - 3):
            #     up = _UpSampling(filters, kernal_size, [2, 1, 1], 'up_%d' % layer, concat_or_add)
            # else:
            up = _UpSampling(filters, kernal_size, [2, 2, 2], 'up_%d' % layer, concat_or_add)
            self.up_layers[dict_key] = up
        ...
        stddev = np.sqrt(2 / (kernal_size ** 2 * root_filters))
        # stddev = 0.02
        self.conv_out = layers.Conv3D(1, 1, padding=padding, use_bias=False,
                                      kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                      name='conv_out')
...
class _UpSampling(Model):
    def __init__(self, filters, kernel_size, pool_size, name, concat_or_add='concat', use_bn=True, use_res=True,
        ...
        stddev = np.sqrt(2 / (kernel_size ** 2 * filters))
        self.deconv = layers.Conv3DTranspose(filters // 2, kernel_size, strides=pool_size, padding=padding,
                                             use_bias=use_bias,
                                             kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                             name='deconv')
        ...
        self.conv2 = layers.Conv3D(filters // 2, kernel_size, padding=padding, use_bias=use_bias,
                                   kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                   name='conv2')
        self.conv3 = layers.Conv3D(filters // 2, 1, padding=padding, use_bias=use_bias,
                                   kernel_initializer=initializers.TruncatedNormal(stddev=stddev),
                                   name='conv3')
